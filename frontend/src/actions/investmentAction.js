...
export const addInvestment = (addData) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.post(`${API_URL}/api/v2/newInvestment`, addData, config);
...
export const updateInvestment=(id,investmentData)=> async(dispatch)=>{
  try{
    ...
    const {data} = await axios.put(`${API_URL}/api/v2/allInvestment/${id}`, investmentData, config);

    dispatch({type:UPDATE_INVESTMENT_SUCCESS, payload:data})
  }catch(error){
    dispatch({type:UPDATE_INVESTMENT_FAIL, payload: error.response.data.message});
  }
}; 
...
export const deleteInvestment=(id)=> async(dispatch)=>{
  try{
    ...
    const {data} = await axios.delete(`${API_URL}/api/v2/allInvestment/${id}`);
    dispatch({type:DELETE_INVESTMENT_SUCCESS, payload:data})
    console.log(data.success);
    
  }catch(error){
    dispatch({type:DELETE_INVESTMENT_FAIL, payload: error.response.data.message});
  }
}; 
